...
let overusedWordsCount = [0, 0, 0]
for (word of storyWords) {
  if (word === overusedWords[0]) {
    overusedWordsCount[0] += 1;
  } else if (word === overusedWords[1]) {
    overusedWordsCount[1] += 1;
  } else if (word === overusedWords[2]) {
    overusedWordsCount[2] += 1;
  }
}
...
const logInfo = (storyWords, sentencesCount, overusedWordsCount) => {
  ...
  console.log('Oversued Word count: ' + overusedWordsCount.reduce((a, b) => a + b));
};

logInfo(storyWords, sentencesCount, overusedWordsCount);
